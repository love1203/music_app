...
class PeopleListsDetailPage extends Component {
    ...
    artistdesc() {
        let res = API.artistdesc({ id: this.detailPeo.id })
            .then(res => {
                if (res.code == 200) {
                    this.setState({
                        artDetail: res.briefDesc
                    })
                }
            })
    }
    ...
    getDetail() {
        let res = API.songtop({ id: this.detailPeo.id })
            .then(res => {
                if (res.code == 200) {
                    console.log(res)
                    this.setState({
        ...
        return <View style={{ flex: 1, backgroundColor: '#fff' }}>
            <View style={{ position: 'relative' }}>
                <View style={{ height: 250, overflow: 'hidden' }}>
                    <Image ref={(img) => { this.backgroundImage = img; }} source={{ uri: this.detailPeo.picUrl }} style={styles.absolute} onLoadEnd={this.imageLoaded.bind(this)} />
                    ...
                    <View style={{ padding: 14, flexWrap: 'wrap', flexDirection: 'row', position: 'absolute', top: 60,display:'flex' }}>
                        <View style={{ width: 90, height: 90, position: 'relative' }}>
                            <Image source={{ uri: this.detailPeo.picUrl }} style={{ width: 90, height: 90, borderRadius: 5, position: "absolute" }} />
                            <View style={{ width: 90, height: 90, borderRadius: 5, position: "absolute", zIndex: -1, backgroundColor: "red" }}></View>
                        </View>
                        <View style={{ flex: 1, marginLeft: 14 }}>
                            <Text style={{ color: '#fff', fontSize: 16, fontWeight: '700' }}>{this.detailPeo.name}</Text>
                            <Text numberOfLines={3} ellipsizeMode={'tail'} style={{ color: '#ccc', fontSize: 13, marginTop: 10 }}>{this.state.artDetail}</Text>
                        </View>
                    </View>
                </View>
